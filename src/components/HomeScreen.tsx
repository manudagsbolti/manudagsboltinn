...
export function HomeScreen({ onNew, onContinue, onSetup, onSummary }: { onNew: () => void; onContinue: (id: string) => void; onSetup: (id: string) => void; onSummary: (id: string) => void }) {
  const data = useLiveQuery(async () => {
    ...
    return { sessions, players, sets, teams, memberships, games, goals, attendance }
  }, [])
  ...
  const totalGoals = data.goals.filter(g => !g.deletedAt).length
  const completedSets = data.sets.filter(s => s.status === 'completed').length
  ...
  const heroStats = data.sessions.filter(s => s.status === 'completed').flatMap(session => {
    ...
    return calculateSessionPlayerStats({
      playerIds: ids,
