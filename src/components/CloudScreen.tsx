...
export function CloudScreen() {
  ...
  const signIn = async () => {
    ...
    setWorking(false); setMessage(error ? error.message : 'Tengdur.')
  }
  const signUp = async () => {
    if (!supabase) return
    setWorking(true); setMessage('')
    const { error } = await supabase.auth.signUp({ email, password })
    setWorking(false); setMessage(error ? error.message : 'Aðgangur stofnaður. Athugaðu tölvupóst ef staðfesting er virk.')
  }
  ...
  const sync = async () => {
    setWorking(true); setMessage('')
    try { const result = await syncCloud(); setMessage(`Sync lokið · ${result.pushed} sent · ${result.pulled} sótt`) }
    catch (error) { setMessage(error instanceof Error ? error.message : String(error)) }
    finally { setWorking(false) }
  }
  ...
  return <section className="screen page-screen cloud-screen">
    ...
    : !auth ? <div className="auth-card card"><h3>Tengjast Mánudagsboltanum</h3><div className="field"><label>Netfang</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} /></div><div className="field"><label>Lykilorð</label><input type="password" value={password} onChange={e=>setPassword(e.target.value)} /></div><button className="primary" disabled={working||!email||!password} onClick={()=>void signIn()}>Skrá inn</button><button disabled={working||!email||!password} onClick={()=>void signUp()}>Stofna aðgang</button></div>
    : <div className="sync-card card"><div><small>Tengdur sem</small><strong>{auth.user.email}</strong></div><div className="queue-count"><b>{queued}</b><span>breytingar í bið</span></div><button className="primary jumbo" disabled={working} onClick={()=>void sync()}>{working?'Samstilli…':'↻ Sync núna'}</button><button className="text-button" onClick={()=>void supabase?.auth.signOut()}>Skrá út</button></div>}
