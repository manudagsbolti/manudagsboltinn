...
export function SeasonPresentationScreen({ seasonId, onBack }: { seasonId: string; onBack: () => void }) {
  const data = useLiveQuery(async () => ({seasons:await db.seasons.toArray(),players:await db.players.toArray(),sessions:await db.sessions.toArray(),attendance:await db.sessionPlayers.toArray(),sets:await db.sets.toArray(),teams:await db.setTeams.toArray(),memberships:await db.setTeamMembers.toArray(),games:await db.games.toArray(),goals:await db.goals.toArray(),backfills:await db.sessionBackfills.toArray()}),[])
  const [slide,setSlide]=useState(0)
  const analytics=useMemo(()=>data?buildSeasonAnalytics(data,data.seasons.find(s=>s.id===seasonId) ?? Number(seasonId)):null,[data,seasonId])
  ...
  return <section className="presentation">
    ...
    <main className={`presentation-slide ${current.kind}`}>
      ...
      {current.kind==='awards'&&<><div className="eyebrow">VERÐLAUN</div><h2>{current.title}</h2><div className="presentation-awards">{current.awards.map((a:any)=><article key={a.key} className={a.tone}><span>{a.icon}</span><small>{a.title}</small><h3>{name(a.playerId)}</h3><strong>{a.value}</strong><p>{a.detail}</p></article>)}</div></>}
