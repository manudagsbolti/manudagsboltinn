...
export function SubmissionReview() {
  ...
  return <section className="card season-context"><h2>Bíður samþykktar</h2>
    ...
    {items.map(i => <button disabled={busy} key={i.id} onClick={() => { setSelected(i); setDate(i.payload.session?.playedOn ?? ''); setSeasonId(''); setRoles({}); setMessage('') }}>{i.payload.session?.playedOn ?? 'Ógild dagsetning'} · Yfirfara kvöld</button>)}
    {selected && <article><h3>Yfirferð kvölds</h3>
      {summary ? <><p>{summary.players.length} leikmenn · {summary.goals} mörk · {summary.completedSets} unnin sett</p><NightSets data={selected.payload}/>
        <label className="field">Leikdagur<input type="date" required value={date} onChange={e => { setDate(e.target.value); setRoles({}) }} /></label>
