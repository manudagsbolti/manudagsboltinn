...
export function SetScoreboard({ set, teams, games, winsPerPoint, pointsToWinSet, setWins = {}, nightWins = {}, playerNames = {} }: { set: SetRecord; teams: SetTeam[]; games: Game[]; winsPerPoint: number; pointsToWinSet: number; setWins?: Record<string, number>; nightWins?: Record<string, number>; playerNames?: Record<string,string> }) {
  ...
  return <div className="set-scoreboard" style={{ gridTemplateColumns: `repeat(${Math.max(teams.length, 1)}, 1fr)` }}>{stats.map(stat => {
    ...
    return <div className={`score-team ${stat.isWinner ? 'winner' : ''}`} key={team.id} style={{ '--team-color': team.color } as React.CSSProperties}>
      ...
      <small>{stat.smallWins} sigrar í setti</small>
      <div className="score-night-totals"><div><b>{setWins[team.id] ?? 0}</b><span>Sett í kvöld</span></div><div><b>{nightWins[team.id] ?? 0}</b><span>Sigrar í kvöld</span></div></div>
      {team.captainPlayerId && <div className="score-captain"><CaptainBadge/><span>{playerNames[team.captainPlayerId] ?? 'Fyrirliði'}</span></div>}
