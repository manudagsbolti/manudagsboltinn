...
export function buildSessionSummary(data: SummaryData) {
  ...
  const teams = new Map<string, { key: string; name: string; color: string; members: string[]; scorers: { playerId: string; name: string; goals: number; ownGoals: number | null; captainSetNos: number[] }[]; setNos: number[]; wins: number; sets: number; goals: number; games: number | null; draws: number | null; ownGoalsReceived: number | null }>()
  for (const team of data.teams.filter(t => setIds.has(t.setId)).sort((a,b) => a.sortOrder-b.sortOrder)) {
    ...
    const row = teams.get(key) ?? { key, name: team.name, color: team.color, members: ids.map(name), scorers: ids.map(playerId => ({ playerId, name: name(playerId), goals: 0, ownGoals: 0, captainSetNos: [] as number[] })), setNos: [], wins: 0, sets: 0, goals: 0, games: 0, draws: 0, ownGoalsReceived: 0 }
    ...
    if (team.captainPlayerId) row.scorers.find(p=>p.playerId===team.captainPlayerId)?.captainSetNos.push(set.setNo)
