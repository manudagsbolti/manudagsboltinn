...
export function buildSessionSummary(data: SummaryData) {
  ...
  const teams = new Map<string, { key: string; name: string; color: string; members: string[]; setNos: number[]; wins: number; sets: number; goals: number; games: number | null }>()
  for (const team of data.teams.filter(t => setIds.has(t.setId)).sort((a,b) => a.sortOrder-b.sortOrder)) {
    ...
    const row = teams.get(key) ?? { key, name: team.name, color: team.color, members: ids.map(name), setNos: [], wins: 0, sets: 0, goals: 0, games: 0 }
  ...
  if (data.backfill) for (const team of data.backfill.teams) {
    teams.set(team.code, { key: team.code, name: team.name, color: team.color, members: team.playerIds.map(name), setNos: data.backfill.rounds.map(r => r.roundNo),
